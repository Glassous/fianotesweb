...
**關於文件位置的說明：**
- **GitHub 筆記倉庫的根目錄**：設置最簡單。
- **`public/` 文件夾**：如果您希望保持 GitHub 筆記倉庫的根目錄整潔，或者您正在部署筆記站點本身並希望靜態提供密碼文件，則可以使用此位置。

> **優先級說明**：如果 GitHub 筆記倉庫的根目錄和 `public/` 文件夾下都存在 `password.fianotes` 文件，將優先使用 **GitHub 筆記倉庫根目錄** 下的密碼文件。

## 自定義根目錄

您可以通過在倉庫根目錄下創建一個名為 `name.fianotes` 的文件來自定義筆記的根目錄。

1.  在根目錄下創建一個名為 `name.fianotes` 的文件。
2.  在文件中寫入您希望作為根目錄的文件夾名稱（例如：`NotesInFia`）。
3.  確保根目錄下存在該名稱的文件夾。

如果 `name.fianotes` 文件和指定的文件夾都存在，應用程序會將該文件夾視為筆記的根目錄。
