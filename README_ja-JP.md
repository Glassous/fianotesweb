...
リポジトリのルートに `name.fianotes` という名前のファイルを作成することで、メモのルートディレクトリをカスタマイズできます。

1.  ルートディレクトリに `name.fianotes` という名前のファイルを作成します。
2.  ファイル内に、ルートとして使用したいフォルダの名前（例：`NotesInFia`）を記述します。
3.  その名前のフォルダがルートディレクトリに存在することを確認してください。

`name.fianotes` ファイルと指定されたフォルダの両方が存在する場合、アプリケーションはそのフォルダをメモのルートディレクトリとして扱います。
