...
| `VITE_ENABLE_AI_PASSWORD` | 启用 AI 密码保护（默认：`false`）。 | 否 |
| `VITE_ENABLE_HASH_PASSWORD` | 启用密码的 SHA-256 哈希校验（默认：`false`）。如果您的密码文件中存储的是哈希值，请设为 `true`。 | 否 |
...
**关于文件位置的说明：**
- **GitHub 笔记仓库的根目录**：设置最简单。
- **`public/` 文件夹**：如果您希望保持GitHub 笔记仓库的根目录整洁，或者您正在部署笔记站点本身并希望静态提供密码文件，则可以使用此位置。

> **优先级说明**：如果 GitHub 笔记仓库的根目录和 `public/` 文件夹下都存在 `password.fianotes` 文件，将优先使用 **GitHub 笔记仓库根目录** 下的密码文件。

## 自定义根目录

您可以通过在仓库根目录下创建一个名为 `name.fianotes` 的文件来自定义笔记的根目录。

1.  在根目录下创建一个名为 `name.fianotes` 的文件。
2.  在文件中写入您希望作为根目录的文件夹名称（例如：`NotesInFia`）。
3.  确保根目录下存在该名称的文件夹。

如果 `name.fianotes` 文件和指定的文件夹都存在，应用程序会将该文件夹视为笔记的根目录。
